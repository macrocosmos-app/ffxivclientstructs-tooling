...
class FfxivClassFactory:
    ...
    def register(self, *args):
        ...
        if arg_types != [int, str, str, dict] and arg_types != [long, str, str, dict]:  # py2
            ...
            if self.parent_class and self.vtbl_size < self.parent_class.vtbl_size:
                print(
                    'Error: The sum of "{0}"\'s parent vtbl sizes ({1}) is greater than the actual class itself ({2})'.format(
                        self.name, self.parent_class.vtbl_size, self.vtbl_size
                    )
                )
                    ...
                    if idx in self.funcs:
                        print(
                            'Warning: 0x{0:X} "{1}" overwrites the name of inherited function "{2}"'.format(
                                self.vtbl_ea, self.name, parent_func_name
                            )
                        )
                        pass
                    else:
    ...
    def _build_vtbl(self):
        ...
        for idx in range(0, self.vtbl_size):
            ...
            if current_func_name.startswith(self.name):
                ...
                print(
                    'Error: Unexpected function name "{0}" at 0x{1:X}'.format(current_func_name, self.vtbl_ea + idx * 8)
                )
...
api.set_addr_name(
    0x1401B0510, "ResourceManager_GetResourceAsync"
...
factory.register(
    0x14167E0F8, "Client::Graphics::Kernel::Shader", "Client::Graphics::Kernel::Resource", {}
)  # its possible shader and buffer are reversed, there's no way to actually tell, not very important
factory.register(
    0x14167E120,
...
factory.register(
    0x1416858D8, "Client::Graphics::Render::PostBoneDeformerBase", "Client::Graphics::RenderObjectList", {}
)
factory.register(
    0x141685938,
...
factory.register(
    0x1416AEBD8, "Client::UI::Misc::RaptureMacroModule", "Client::UI::Misc::UserFileManager::UserFileEvent", {}
)
